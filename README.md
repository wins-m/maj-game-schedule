...
- **默认配置**：8人参赛，4人每桌
- **比赛场次**：默认6场积分赛，每场2桌
- **初次分桌**：支持手动设置、积分排序、随机洗牌分组

### 2. 积分赛管理
- 每场积分赛结束后，手动录入每位选手的积分（支持浮点数）
- 自动更新个人总积分
- 根据积分排名自动决定下一场分桌情况
- 支持选手重命名功能
...
### 4. 历史记录管理
- **历史记录查看**：查看已完成比赛的详细记录
- **筛选功能**：支持按场次、桌次、选手筛选历史记录
- **编辑功能**：可编辑历史记录的积分和牌谱网址
- **导出导入**：支持历史记录的JSON格式导出和导入

### 5. 数据备份与恢复
- **自动备份**：选手保存时间表时自动创建备份文件
- **备份文件**：按日期命名，便于版本管理
- **数据持久化**：支持多设备数据同步
...
- **文件锁管理**：并发访问保护
- **备份系统**：自动备份机制
- **优势**：支持多设备、多浏览器数据同步，支持浮点积分、历史记录管理
...
- **POST /api/players** - 更新选手信息
- **GET /api/games** - 获取比赛列表
- **POST /api/games** - 创建新比赛
- **PUT /api/games/:round** - 更新比赛结果
- **GET /api/schedules/:playerId** - 获取选手时间表
- **POST /api/schedules/:playerId** - 保存选手时间表
...
│   ├── schedule.js        # 时间表管理逻辑
│   ├── history.js         # 历史记录管理逻辑
│   ├── storage.js         # 数据存储管理（前端）
│   ├── storage-api.js     # 数据存储API（后端）
│   └── utils.js           # 工具函数
├── server/                 # Node.js后端服务
│   ├── app.js             # 后端主文件
...
2. **初次分桌**：选择分组方式（积分排序/随机洗牌）分配第一场分桌
3. **进行比赛**：每场结束后录入积分（支持小数点）
4. **选手管理**：可随时重命名选手姓名
5. **填写时间表**：选手点击自己的名字，勾选空闲时间（保存时自动备份）
6. **查看共同时间**：当一桌4人都填表后，自动显示共同空闲时间
7. **继续下一场**：系统自动根据积分排名分桌
8. **查看历史**：访问历史记录页面，查看/编辑/导出已完成的比赛记录
